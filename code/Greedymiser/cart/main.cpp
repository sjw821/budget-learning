...
int main(int argc, char* argv[]) {
 ...
 if (myargs.processors==1)
  ...
  for (int round = 0; round < myargs.rounds; round++)
  {
    ...
    if (myargs.read_costs)
      read_costs(myargs, myargs.features);
    if (myargs.read_targets)
      read_targets(train, myargs);
    // ***do algorithm*** //

    // if (myargs.read_targets && myargs.read_costs)
    //   {for(i=0; i<myargs.features; i++)  cout <<"feature : " <<myargs.costs[i] << endl;}
  	// if (myargs.read_targets)
  	//   for(i=0; i<train.size()-1; i++)
  	//     cout << "line : " << train[i]->label  << endl;
  	// if (myargs.read_targets)
  	//   for (i=0; i<myargs.num_test; i++)
  	//     for(int j = 0; j < test_preds[i].size(); j++)
  	//       cout <<"teste : " <<test_preds[i][j] << endl;
    // return 0;
  ...
  if (myargs.read_targets && myargs.read_costs)
    {for(i=1; i<myargs.features; i++)  cout << myargs.costs[i] << endl;}
	if (myargs.read_targets)
	  for(i=0; i<N; i++)
	...
	// if combining with python, print predictions of this tree to stdout
      if (myargs.read_targets) {
  if (myargs.read_costs){for(i=1; i<=myargs.features; i++)  cout << myargs.costs[i] << endl;}
	for(i=0; i<N; i++)
	  cout << train_preds[i] << endl;
      	for (i=0; i<myargs.num_test; i++)
	  for(int j = 0; j < test_preds[i].size(); j++)
