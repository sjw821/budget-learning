...
int load_data(vector<tuple*>& train, vector< vector<tuple*> >& test,  args_t& myargs) {
...
void read_targets(vector<tuple*>& train,  args_t& myargs) {
  int N = train.size(), i;
...
void read_weights(vector<tuple*>& train,  args_t& myargs) {
  int i, N=train.size();
  for (i=0; i<N; i++) {
    double w; cin >> w;
    train[i]->weight = w;
...
void read_costs(args_t& myargs, int f) {
  int i, N=f;
  ...
  // fprintf(stderr,"costs read");
}
// free all the memory we used up
void free_memory( args_t& myargs, vector<tuple*>& train, vector< vector<tuple*> >& test) {
  int i;
...
void presort(data_t train, args_t& myargs){
	int numfeatures = myargs.features;
...
void presort_in_range(data_t train,  args_t& myargs, int start, int end){
        // sort data
...
void presort_p(data_t train,  args_t& myargs) {
	int numfeatures = myargs.features;
	int n = train.size();
	...
	for (int i = 0; i < numthreads; i ++)
		threads[i] = new thread(bind(presort_in_range, train, ref(myargs), i*(numfeatures-1)/numthreads, (i+1)*(numfeatures-1)/numthreads));
