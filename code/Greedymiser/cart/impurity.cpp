...
double best_fc_in_feature(vector<tuple*> data, vector<int> dataCount, vector<int> invertIdx, int f, int& fs, double& vs,  args_t& args) {  //if (!XX)
...
//void find_split_in_range(vector<tuple*> data, vector<int> dataCount, vector<int> invertIdx, int start, int  end, int& fs, double& vs, double& I, vector<bool> skip, args_t& args) {
void find_split_in_range(vector<tuple*> data, vector<int> dataCount, vector<int> invertIdx, pair<int,int> range, int& fs, double& vs, double& I, vector<bool>skip,  args_t& args) {
								...
								for (int i = start; i < end; i++) {
																...
																int fi;
																double vi, Ii;
																Ii = best_fc_in_feature(data, dataCount, invertIdx, f, fi, vi, args)+args.costs[f];
																if (Ii < min) {
																								min = Ii;
																								fs = fi;
																								vs = vi;
...
bool find_split_p(vector<tuple*> data, vector<int> dataCount, vector<int> invertIdx, int NF, int& f_split, double& v_split, vector<bool>& skip, args_t& args) {
								...
								for (i = 0; i < numthreads; i++)
																...
																threads[i] = new thread(bind(find_split_in_range, data, dataCount, invertIdx, make_pair(i*(NF-1)/numthreads, (i+1)*(NF-1)/numthreads), ref(F[i]), ref(V[i]), ref(Imp[i]), ref(skip),ref(args) ));
								for (i = 0; i < numthreads; i++) {
																threads[i]->join();
																delete threads[i];
								}
...
bool dt_node::entropy_split(data_t data, vector<int> dataCount, vector<int> invertIdx, int NF, int& f_split, double& v_split, int K, bool par,  args_t& args) {
								f_split = -1;
								double min = MY_DBL_MAX;
								int n = data.size(), i;
								...
								for (int f = 1; f < NF; f++) {
																...
																for (i = start; i < n-1; i++) {
																								...
																								double I = W_left/ssum * L +  W_right/ssum * R + W_miss/ssum * M+ args.costs[f];
																								//I = 1.0*n_left/n *
...
bool dt_node::find_split(vector<tuple*> data, vector<int> dataCount, vector<int> invertIdx, int NF, int& f_split, double& v_split, int K, bool par , args_t& args) {
								if (args.loss == ALG_ENTROPY)
																return entropy_split(data, dataCount, invertIdx, NF, f_split, v_split, K, par,args);
