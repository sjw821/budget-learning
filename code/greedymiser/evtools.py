...
def rel2ranks(rels,qs):
    ...
    def printtable(table):
        inds=list(table.items());
        inds.sort(key=operator.itemgetter(1),reverse=True);
        pos=dict(list(zip(list(map(operator.itemgetter(0),inds)),list(range(1,len(inds)+1)))));
        return(' '.join([str(pos[i]) for i in range(1,len(inds)+1)]))

    counter=1;
    curid=-1;
    for (p,qid) in zip(rels,qs):
...
def rel2labels(rels,qs):
    ...
    def printtable(table):
        ...
        return(' '.join([str(a_b[1]) for a_b in inds]))
...
def getrmse(preds,labels):
	mse=0.0;
	for (pr,ta) in zip(preds,labels):
	    mse+=(pr-ta)**2;
	return(sqrt(mse/float(len(preds))))


def evaluate(preds,queries,labels):
