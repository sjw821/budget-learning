...
cmdline = './cart/rtrank %s %s /dev/null -f %i -r %i -d %i -z -p %i' %  (INPUTDATA,TESTDATA,NUM_FEATURES,ITER,DEPTH,PROCESSORS)
...
for itr in range(ITER):
    ...
    for i in range(nall):
        l=p.stdout.readline()
        preds[i] += STEPSIZE*float(l.decode('UTF-8').split(' ',1)[0])
