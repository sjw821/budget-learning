...
public:
  ...
  dt_node(const vector<tuple*>& data, args_t& myargs, int maxdepth, int depth, int fk, bool par, args_t& margs) : leaf(0), feature(0), value(UNKNOWN), pred(-1), args(myargs){
	...
	vector<int> invertIdx(myargs.ntra, -1);
	vector<int> countData(myargs.ntra,0);
    ...
    if (!find_split(data, countData, invertIdx, myargs.features, f_split, v_split, fk, par,args)) {
      leaf = true;
      return;
    }
    ...
    feature = f_split;
    value = v_split;
    // seet feature cost to 0 for the future
    args.costs[feature]=0;
  ...
  static bool find_split(vector<tuple*> data, vector<int> countData, vector<int> invertIdx, int NF, int& fsplit, double& vsplit, int K, bool par,  args_t& args);
  ...
  static bool entropy_split(data_t data, vector<int> countData, vector<int> invertIdx, int NF, int& fsplit, double& vsplit, int K, bool par,  args_t& args);
  ...
  static void split_data(const vector<tuple*>& data, vector<tuple*> child[CHILDTYPES], int f, double v,  args_t& args) {
    int n = data.size(), i;
  ...
  static double classify_all(const data_t& test, const dt_node* const tree, preds_t& preds,  args_t& args) {
    int i, N = test.size();
    double r = 0.0;
    for (i=0; i<N; i++) {
